...
export const CATEGORY_FALLBACK_IMAGES: Record<string, string> = {
  ...
  'Breakfast-non_veg': '/assets/smoked_chicken_tikka.jpg',

  'Desserts-veg': '/assets/gulab_jamun.jpg',
...
export const KNOWN_BROKEN_URLS: Record<string, string> = {
  // previous generic roll/dish URL -> authentic tandoori smoked chicken tikka photo
  'https://images.unsplash.com/photo-1627308595229-7830a5c91f9f': 
    '/assets/smoked_chicken_tikka.jpg',
  ...
  'https://images.unsplash.com/photo-1599488615731-7e5c2823ff28?auto=format&fit=crop&w=600&q=80':
    '/assets/gulab_jamun.jpg',
  'https://images.unsplash.com/photo-1601050690113-d49cb376e1a9?auto=format&fit=crop&w=600&q=80':
    '/assets/gulab_jamun.jpg'
};
...
export function sanitizeFoodImageUrl(
  ...
  if (foodName) {
    ...
    if (nameLower.includes('jamun') || nameLower.includes('gulab')) {
      return '/assets/gulab_jamun.jpg';
    }
    if (nameLower.includes('smoked chicken') || (nameLower.includes('chicken') && nameLower.includes('tikka'))) {
      return '/assets/smoked_chicken_tikka.jpg';
    }
    // Dedicated distinct photos for Veg Biryani vs Non-Veg Biryani
    if (nameLower.includes('biryani') || nameLower.includes('biriyani') || nameLower.includes('pulao')) {
      if (dietary === 'veg' || nameLower.includes('veg') || nameLower.includes('paneer') || nameLower.includes('soya')) {
        return '/assets/veg_biryani.jpg';
      }
      return '/assets/chicken_biryani.jpg';
    }
  }
  ...
  // Active separation for Biryani photos: ensure Veg Biryani never uses Chicken Biryani photo
  if (dietary === 'veg' && (url.includes('1563379091339') || url.includes('chicken_biryani'))) {
    return '/assets/veg_biryani.jpg';
  }
  if ((dietary === 'non_veg' || foodName?.toLowerCase().includes('chicken')) && url.includes('1563379091339')) {
    return '/assets/chicken_biryani.jpg';
  }

  // Previous chicken roll URL mapped to authentic photo
  if (url.includes('1627308595229')) {
    return '/assets/smoked_chicken_tikka.jpg';
  }
